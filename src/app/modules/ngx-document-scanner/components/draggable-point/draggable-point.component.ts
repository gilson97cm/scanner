import { LimitsService, PositionChangeData, PointPositionChange } from './../../services/limits.service';
import { ImageDimensions } from './../../PublicModels';
import { XYPosition, LimitException, PointOptions } from './../../PrivateModels';
...
export class DraggablePointComponent implements AfterViewInit {
  ...
  @Input() pointOptions: PointOptions = { color:'', height: 0, width: 0, shape: 'rect' }// 'rect' | 'circle' = 'rect';
  @Input() limitRoles: Array<'left' | 'right' | 'top' | 'bottom'> = [];
  @Input() startPosition: XYPosition = { x: 0, y: 0 };
  @Input() container: HTMLElement = {} as HTMLElement;
  @Input() private _currentPosition: XYPosition = { x: 0, y: 0 };
  position: XYPosition = {
    x: 0,
    y: 0
  };
  private _paneDimensions: ImageDimensions = { height: 0, width: 0 };
  resetPosition: XYPosition = { x: 0, y: 0 };
  ...
  private adjustPosition(limitException: LimitException) {
    ...
    Object.keys(this.startPosition).forEach(axis => {
      let key: keyof XYPosition = axis == 'x' ? 'x' : 'y'
      newPosition[key] = limitException.resetCoordinates[key] + limitException.resetCoefficients[key];
    });
